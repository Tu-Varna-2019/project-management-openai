...
export function TicketClass(props) {
    const { navigate, location, getProjectID } = useContext(ProjectContext);
    const { currentUser } = useContext(UserContext);
    const { sprintID, sprintNumbers, sprintNum, PIID, setPIID, setPINum } =
        useContext(PISprintContext);

    // Ticket/s
    const editTicket = location.state ? location.state.selectedTicket : "";

    const backlogTicketBoolean = location.state
        ? location.state.edited_bg
        : false;
    ...
    const createIssueTemplateBoolean = location.state
        ? location.state.create_it
        : false;
    const editIssueTemplateBoolean = location.state
        ? location.state.edited_it
        : false;
    ...
    const watchedCount = watchedUsers?.match(/,/g)
        ? watchedUsers.match(/,/g).length
        : 0;
    const ticketStatusColorVariant =
        ticketStatus === "ToDo"
            ? "error"
                ...
                await DataStore.query(PI, PIID).then((PIObj) => {
                    PIObj.Sprints.values.then((sprintsArray) => {
                        sprintsArray.forEach(async (item) => {
                            await DataStore.query(Sprint, item.id).then(
                                (sprintIter) => {
                                    const ticketArr = [];
                                    sprintIter.Tickets.values.then(
                                        (TicketsArray) => {
    ...
    useEffect(() => {
        if (
            getNotificationCountState() !== null &&
    ...
    useEffect(() => {
        async function fetchUserData() {
            await DataStore.query(Ticket)
                .then((data) => {
                    setTickets(
                        data.filter((item) => item.sprintID === sprintID)
                    );
                })
                .catch((error) => {
                    console.error(error);
    ...
    useEffect(() => {
        setTicketToDo(
            Object.values(tickets).filter(
                (item) => item.TicketStatus === "ToDo"
            )
        );
        setTicketInProgress(
            Object.values(tickets).filter(
                (item) => item.TicketStatus === "InProgress"
            )
        );
        setTicketInReview(
            Object.values(tickets).filter(
                (item) => item.TicketStatus === "InReview"
            )
        );
        setTicketDone(
            Object.values(tickets).filter(
                (item) => item.TicketStatus === "Done"
            )
        );
    }, [tickets]);
    ...
    useEffect(() => {
        setEpicLinkOptions([
            ...
            Object.keys(tickets)
                ...
                .map(
                    (key) =>
                        `KAI-${tickets[key].TicketID}: ${tickets[key].Title}`
                )
        ]);
                ...
                .then((data) => {
                    data.filter((item) => {
                        if (item.sub !== "00000000") {
                            setPeopleAssign((prevList) => [
                                ...new Set([...prevList, item.username])
                            ]);
                            setPeopleAssignSub((prevList) => [
                                ...new Set([...prevList, item.sub])
                            ]);
                        }
                        return item;
                    });
        ...
        async function fetchUserData() {
            if (editTicket !== "") {
                try {
                    ...
                    setDescription(
                        editTicket.Description === null
                            ? ""
                            : editTicket.Description
                    );
                    setComment(
                        editTicket.Comment === null ? "" : editTicket.Comment
                    );
                    setImageTicket(
                        editTicket.ImageTicket === null
                            ? ""
                            : editTicket.ImageTicket
                    );
                    ...
                    setStoryPoint(
                        editTicket.StoryPoint === null
                            ? 0
                            : editTicket.StoryPoint
                    );
                    setCreatedDate(new Date(editTicket.CreatedDate));
                    setUpdatedDate(
                        editTicket.UpdatedDate === null
                            ? "-"
                            : new Date(editTicket.UpdatedDate)
                    );
                    setResolvedDate(
                        editTicket.ResolvedDate === null
                            ? "-"
                            : new Date(editTicket.ResolvedDate)
                    );
                    setEpicLink(editTicket.EpicLink);
                    setWatchedUsers(
                        editTicket.Watch === null ? "" : editTicket.Watch
                    );
                    ...
                    await DataStore.query(User).then((data) => {
                        data.filter((item) => {
                            if (item.sub === editTicket.Asignee) {
                                ...
                                Storage.get("shared/" + item.ImageProfile, {
                                    level: "public"
                                }).then((data_url) => {
    ...
    useEffect(() => {
        async function fetchUserData() {
            await DataStore.query(Ticket).then((data) => {
                data.filter((item) => {
                    if (
    ...
    const handleIssueType = async (event) => {
        ...
        const issueTypeTaskSub =
            event.target.value === "Task" ? "Subtask" : "Task";
            ...
            case "Subtask":
                try {
                    ...
                    for (const item_ticket of data_ticket) {
                        const data_sprints = await DataStore.query(
                            Sprint,
                            item_ticket.sprintID
                        );
                        const data_PI = await DataStore.query(
                            PI,
                            data_sprints.piID
                        );
                        if (
                            data_PI !== undefined &&
                            data_PI.projectID === getProjectID
                        ) {
                            if (
                                item_ticket.IssueType === issueTypeTaskSub &&
                                item_ticket.id !== isEditedTicketID &&
                                (!subtasks ||
                                    !subtasks.includes(item_ticket.id))
                            ) {
            ...
            .then((hashBuffer) => {
                ...
                setImageTicket((prevValue) => {
                    ...
                    if (parts.length <= 10) {
                        return (
                            prevValue +
                            `${hashHex}${randomString}.${fileExtension},`
                        );
                    } else
                        Swal.fire(
                            "Error!",
                ...
                return {
                    file,
                    key: `${hashHex}${randomString}.${fileExtension}`
                };
            });
    };
        ...
        if (deletedImageName !== "" && deletedImageName !== undefined) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    await Storage.remove("shared/" + deletedImageName, {
                        level: "public"
                    });
                    setImageTicket(
                        imageTicket.replace(deletedImageName + ",", "")
                    );
                    setAttachmentUrls([]);
                    setAttachmentName([]);
                    const editTicketDataStore = await DataStore.query(
                        Ticket,
                        editTicket.id
                    );
                    await DataStore.save(
                        Ticket.copyOf(editTicketDataStore, (item) => {
                            item.ImageTicket = imageTicket.replace(
                                deletedImageName + ",",
                                ""
                            );
                        })
    ...
    const handleMoreOptionsChange = async (event) => {
        ...
        const Sprints = sprintSlice.filter(
            (item) => item !== parseInt(sprintNum)
        );

        const timezoneOffset = new Date().getTimezoneOffset() * 60000;
        const newCreatedDate = new Date(
            new Date(createdDate).getTime() - timezoneOffset
        );
        ...
        switch (event.target.value) {
            ...
            case moreOptions[1]:
                if (resolvedDate !== "-") {
                    const newResolvedDate = new Date(
                        new Date(resolvedDate).getTime() - timezoneOffset
                    );
                ...
                setTimeout(() => {
                    navigate(location.pathname, {
                        state: {
                            ...
                            alert_description: `${title} has been successfully cloned!`
                        }
                    });
                        ...
                        setTimeout(() => {
                            navigate(location.pathname, {
                                state: {
                                    ...
                                    alert_show: "block",
                                    alert_variant: "success",
                                    alert_description: `${title} has been successfully deleted!`
                                }
                            });
                            window.location.reload();
                        }, 1200);
            ...
            case moreOptions[3]:
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const movedTicket = await DataStore.query(
                            Ticket,
                            editTicket.id
                        );
                        ...
                        await DataStore.save(
                            Ticket.copyOf(movedTicket, (item) => {
                        ...
                        setTimeout(() => {
                            navigate(location.pathname, {
                                state: {
                                    ...
                                    alert_show: "block",
                                    alert_variant: "success",
                                    alert_description: `${title} has been successfully moved to : Sprint ${
    ...
    const handleCloseEditTicketClick = (event) => {
        navigate(location.pathname, {
            state: { edited: false, project: getProjectNameState() }
        });
    };
    ...
    const handleSaveEditTicketClick = async (event) => {
        ...
        if (ticketStatus === "Done") {
            const newResolvedCurrentDate = new Date(
                new Date().getTime() - timezoneOffset
            );
        ...
        try {
            ...
            const checkEditTicketSubs =
                editTicket.Subtasks !== null ? editTicket.Subtasks : [];
            await DataStore.save(
                Ticket.copyOf(editTicketDataStore, (item) => {
                    item.Title = title;
            ...
            // ADD
            if (selectedTaskID.trim() !== "") {
                const editParentChildTaskDataStore = await DataStore.query(
                    Ticket,
                    selectedTaskID
                );
                ...
                const checkEditTicketSubs =
                    editParentChildTaskDataStore.Subtasks !== null
                        ? editParentChildTaskDataStore.Subtasks
                        : [];
                await DataStore.save(
                    Ticket.copyOf(editParentChildTaskDataStore, (item) => {
            ...
            // DELETE
            if (deletedTaskID.length > 0) {
                const editParentTaskDataStore = await DataStore.query(
                    Ticket,
                    editTicket.id
                );
                ...
                deletedTaskID.map(async (deleted_id) => {
                    ...
                    const ticketToModify = await DataStore.query(
                        Ticket,
                        deleted_id
                    );
                    ...
                    await DataStore.save(
                        Ticket.copyOf(ticketToModify, (item_link) => {
                            if (item_link.Subtasks !== null)
                                item_link.Subtasks = item_link.Subtasks.filter(
                                    (subs) => subs !== editTicket.id
                                );
                        })
                    );
                });
            ...
            if (watchedUsers !== "" || asigneeName !== "Unassigned") {
                const concatenateWatchUserWithAsignee =
                    !watchedUsers.includes(asigneeName + ",") &&
                    isEmailValid.test(asigneeName)
                        ? watchedUsers + asigneeName + ","
                        : watchedUsers === ""
                        ? asigneeName + ","
                        : "";
                const newTicket = {
                    ...
                    TicketID: ticketID,
                    StoryPoint: storyPoint,
                    Reporter: isEmailValid.test(reporterName)
                        ? reporterName
                        : "",
                ...
                try {
                    ...
                    notify_update_ticket_response =
                        "Unable to send message , sorry for the inconvinience!";
                }
            }
            setTimeout(() => {
                navigate(location.pathname, {
                    state: {
                        ...
                        alert_show: "block",
                        alert_variant: "success",
                        alert_description: `${title} has been successfully edited : \n ${notify_update_ticket_response} `
                    }
                });
                window.location.reload();
            }, 1200);
            ...
            navigate(location.pathname, {
                state: {
                    ...
                    alert_variant: "error",
                    alert_description:
                        "App is not supported in this browser's private mode! Please enable cookies!"
                }
            });
        }
    };
    ...
    const handleAssignToMeClick = async (event) => {
        event.preventDefault();
        await Storage.get("shared/" + currentUser.ImageProfile, {
            level: "public"
        }).then((data_url) => {
            setAsignee(
                peopleAssignSub[peopleAssign.indexOf(currentUser.username)]
            );
            setAsigneeImageURL(data_url);
    ...
    const handleAddUserToWatch = (event) => {
        ...
        if (watchedAddMeVariant === "success") {
            ...
            setWatchedUsers(
                (prevValue) => prevValue + currentUser.username + ","
            );
        ...
        try {
            ...
            if (asigneeName !== "Unassigned") {
                const newTicket = {
                    ...
                    TicketID: ticketID,
                    StoryPoint: storyPoint,
                    Reporter: isEmailValid.test(currentUser.username)
                        ? currentUser.username
                        : "",
                    Watch: isEmailValid.test(asigneeName)
                        ? asigneeName + ","
                        : "",
                    Asignee: isEmailValid.test(asigneeName) ? asigneeName : "",
                    EpicLink: epicLink,
            ...
            navigate(location.pathname, {
                state: {
                    ...
                    alert_show: "block",
                    alert_variant: "success",
                    alert_description: `${title} has been successfully created! : ${notify_create_ticket_response}`
                }
            });
            window.location.reload();
        } catch (error) {
            ...
            navigate(location.pathname, {
                state: {
                    ...
                    alert_show: "block",
                    alert_variant: "error",
                    alert_description: `App is not supported in this browser's private mode! Please enable cookies! ${error}`
                }
            });
            window.location.reload();
        }
    ...
    const handleReleaseMoveTicket = async (draggedTicketID, boardStatus) => {
        ...
        if (
            ...
            if (editTicketDataStore.TicketStatus !== boardStatus) {
                ...
                const isNotifStateNull =
                    getNotificationsState() === null
                        ? ""
                        : getNotificationsState();
                setNotificationsState(
                    `${isNotifStateNull},${currentUser.username} : ${editTicketDataStore.Title} : ${ticketStatus} --> ${boardStatus}`
                );

                setNotificationCountState(+getNotificationCountState() + 1);

                await DataStore.save(
                    Ticket.copyOf(editTicketDataStore, (item_edit_status) => {
