...
export function User2Class() {
    ...
    const defaultUserImageSHA =
        "ZGVmYXVsdF91c2VyX3Byb2ZpbGUuZGVmYXVsdF91c2VyX3Byb2ZpbGUucG5n.png";

    let selectedUserID = "";
    let selectedUsername = "";
    const addRemoveUserBoolean = location.state
        ? location.state.add_remove_user
        : false;
    try {
                ...
                try {
                    ...
                    if (currentCredentials !== undefined) {
                        ...
                        const isUserFound = users.find(
                            (user) =>
                                user.sub === currentCredentials.attributes.sub
                        );

                        if (isUserFound) {
                            setCurrentUser(isUserFound);
                        } else {
                            const checkUserGoogle =
                                currentCredentials.attributes.email ===
                                undefined
                                    ? `Google_${generate({
                                          minLength: 5,
                                          maxLength: 25
    ...
    useEffect(() => {
        async function fetchUserData() {
            await Storage.vault
                .get("shared/" + currentUser.ImageProfile, {
                    level: "public"
                })
                .then((data) => {
    ...
    useEffect(() => {
        async function fetchUserData() {
            if (selectedUserID !== currentUser.id) {
                ...
                await Storage.vault
                    .get("shared/" + viewedUser.ImageProfile, {
                        level: "public"
                    })
                    .then((data) => {
                        setViewedUserProfileURL(data);
                    });
            } else setViewedUserProfileURL(userProfileURL);
        }
        fetchUserData();
    ...
    useEffect(() => {
        try {
            if (location.state.alert_variant !== undefined) {
                ...
                setAlertVisibility(
                    location.state ? location.state.alert_show : "none"
                );
    ...
    const handleSaveEmailClick = async (event) => {
        try {
            ...
            const verificationCode = prompt(
                `Message sent ${email} . Please enter the verification code`
            );
            ...
            const editUserDataStore = await DataStore.query(
                User,
                currentUser.id
            );
            await DataStore.save(
                User.copyOf(editUserDataStore, (item) => {
            ...
            navigate(location.pathname, {
                state: {
                    ...
                    alert_description: `${email} has been successfully edited!`
                }
            });
        ...
        setTimeout(() => {
            navigate(location.pathname, {
                state: {
                    ...
                    alert_variant: "success",
                    selectedUserID: currentUser.id,
                    alert_description: "Image successfully uploaded!"
                }
            });
            window.location.reload();
        }, 1200);
    ...
    const handleGoToMNotes = (event) => {
        navigate(location.pathname, {
            state: { add_remove_user: true, selectedUserID: currentUser.id }
        });
    };
    ...
    const handleAdminUserItemClick = (user_id) => {
        ...
        if (!selectedAdminUser.includes(user_id))
            ...
            setSelectedAdminUser(
                selectedAdminUser.filter((item) => item !== user_id)
            );
    };
